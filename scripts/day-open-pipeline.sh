...
#
# FMT promotion note (WP-7 FMT-PROMOTE-DAYOPEN1): this entry point is promoted to
# FMT-exocortex-template/scripts/, but 6 of the 7 scripts it calls are not (see
# day-open-checks-runner.sh, day-open-llm-fill.py, day-open-budget-patch.py,
# generate-day-report.sh, update-derived-snapshot.py, llm-proxy-launcher.sh) — the
# promoted copy will fail on first LLM-fill call until those are promoted too.
