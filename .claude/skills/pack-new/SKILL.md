...
**Короткий код (`pack_id_code`, WP-474 Ф3-фикс).** Отдельно от `pack_id_slug` (kebab-case, для имени директории) — короткий мнемо-код из 2-4 заглавных латинских букв, используемый как префикс в кодах сущностей (`{{PACK_ID}}.D.NNN` → например `DP.D.NNN`). Это два разных идентификатора — не путать: полные существующие Pack используют именно короткий код в этой роли (`PACK-digital-platform` → код `DP`), а не полный slug. Предложить 1-2 варианта кода (аббревиатура от домена) вместе со slug на этом шаге, дать пилоту выбрать/поправить. Код тоже provisional — финализируется вместе со slug (см. «Финализация имени» ниже).

**Антипримеры кода:**
- Совпадение с уже существующим кодом другого Pack в IWE — проверить `grep -rl "^pack_id: <код>$" ~/IWE/PACK-*/pack/*/00-pack-manifest.md` перед фиксацией
